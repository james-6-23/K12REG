...
def _random_name() -> tuple[str, str]:
    # Prefer shared pool from browser path (large + de-duped).
    try:
        from register.browser_registrar import _random_name as _bn
...
class PlatformRegistrar:
    ...
    def _cf_retry(self, url: str, make_request, label: str = "") -> Any:
        ...
        (JSD pure-compute first, then FlareSolverr) and retries once.
        """
        ...
        if not self.jsd_url and not self.flaresolverr_url:
            status = getattr(resp, "status_code", "?")
            raise RegistrationError(
                f"[{label}] Cloudflare block (no JSD/FlareSolverr). "
                f"HTTP {status}. Set proxy.jsd_url=http://127.0.0.1:8191 "
                f"(start JSD: cd JSD && ./jsd-solver) or proxy.flaresolverr_url."
            )
        ...
        if not refresh_clearance_and_retry(
            ...
            raise RegistrationError(
                ...
                f"(JSD/FlareSolverr). Same proxy as solve is required."
            )
    ...
    def _register_user(self, email: str, password: str, index: int) -> None:
        ...
        def _do():
            # Rebuild sentinel on every attempt (including CF clearance retry).
            headers = json_headers(
                f"{AUTH_BASE}/create-account/password", self.device_id
            )
            sentinel_val, _oai_sc, so_val = build_sentinel_bundle(
            ...
            # retry_attempts=1: never auto-resend this POST. It creates the
            # account server-side, so a network blip after the server processed
            # the first send would otherwise trigger a duplicate submit and a
            # misleading "user_already_exists" on the retry (orphaned account).
            return request_with_retry(
                self.session,
                "post",
                url,
    ...
    def _validate_otp(self, code: str, index: int) -> None:
        ...
        # Retry with fresh sentinel token (and so-token when present)
        sentinel_val, _oai_sc, so_val = build_sentinel_bundle(
    ...
    def _create_account(self, name: str, birthdate: str, index: int) -> None:
        ...
        def _do():
            # CRITICAL: rebuild sentinel after CF clearance — never reuse tokens.
            # Contract:
            #   openai-sentinel-token    = {p, t, c, id, flow}  (t non-empty when required)
            #   openai-sentinel-so-token = {so, c, id, flow}    when so present
            #   oai-sc cookie            set inside build_sentinel_bundle
            ...
            if so_val:
                headers["openai-sentinel-so-token"] = so_val
            # retry_attempts=1: never auto-resend. create_account is
            # non-idempotent — a resend after the server already created the
            # account returns "user_already_exists" and orphans it.
            return request_with_retry(
                self.session,
                "post",
                url,
    ...
    def register(self, index: int = 0, mailbox: dict | None = None) -> dict:
        ...
        ``mailbox`` may be pre-allocated by the pipeline for live-board email bind.
        """
        ...
        try:
            ...
            plog.board_bind(index, email, note="protocol · start…")
        except Exception:
            plog = None  # type: ignore

        def _status(note: str) -> None:
            if plog is None:
                return
            try:
                plog.board_status(index, note, email=email)
            except Exception:
                pass
        ...
        try:
            ...
            proxy_hint = ""
            if self.proxy:
                # Keep host:port only — no credentials in the board.
                try:
                    from urllib.parse import urlparse
                    ...
                    if host:
                        proxy_hint = f" · proxy {host}{port}"
                except Exception:
                    proxy_hint = ""
            _status(f"authorize · PKCE signup{proxy_hint}…")
            self._platform_authorize(email, index)

            # Step 5: Register user
            _status(f"register user · {first_name} {last_name}…")
            self._register_user(email, password, index)

            # Step 6: Send OTP
            _status("send OTP · auth.openai.com…")
            self._send_otp(index)

            # Step 7: Wait for verification code
            mail_kind = str(
                self.mail_config.get("provider")
                or self.mail_config.get("type")
                or "mail"
            ).strip() or "mail"
            _status(f"wait OTP · {mail_kind} inbox…")

            def _otp_tick(elapsed: float, timeout: float) -> None:
                _status(
                    f"OTP wait · {mail_kind} · {elapsed:.0f}/{timeout:.0f}s"
                )

            try:
                code = wait_for_code(
                    self.mail_config, mailbox, on_tick=_otp_tick
                )
            except TypeError:
                # Older wait_for_code without on_tick
                code = wait_for_code(self.mail_config, mailbox)
            if not code:
                raise RegistrationError("Timed out waiting for verification code")
            logging.getLogger(__name__).info(
                f"[Task {index}] Got verification code: {code}"
            )
            _status(f"OTP got {code} · validate…")

            # Step 8: Validate OTP
            _status(f"validate OTP · code {code}…")
            self._validate_otp(code, index)

            # Step 9: Create account
            full_name = f"{first_name} {last_name}"
            birthdate = _random_birthdate()
            _status(f"create account · {full_name} · dob {birthdate}…")
            self._create_account(full_name, birthdate, index)

            # Step 10: Exchange tokens
            _status("exchange tokens · oauth code → AT/RT…")
            tokens = self._exchange_tokens(index)
            at_ok = bool(str(tokens.get("access_token") or "").strip())
            rt_ok = bool(str(tokens.get("refresh_token") or "").strip())
            _status(
                f"access_token ok · AT={'yes' if at_ok else 'no'} "
                f"RT={'yes' if rt_ok else 'no'}"
            )

        except Exception as error:
            mark_mailbox_result(mailbox, success=False, error=error)
            _status(f"fail · {str(error)[:96]}")
            raise
...
def register_worker(
    ...
    try:
        result = registrar.register(index, mailbox=mailbox)
